...
│  │  │ tg|fs|qq|dt|ww    │◄──│    核心    │──►│ claude | opencode | codex │  │  │
│  │  │     adapters      │   │  （调度层）│   │          runners           │  │  │
│  │  └───────────────────┘   └────────────┘   └─────────────────────────────┘  │  │
│  │          │                   │                  │          │  │
...
internal/
  ...
  agent/
    ...
    opencode/
      runner.go             启动 opencode run --format json 子进程
      parser.go             JSONL 解析器，处理 opencode 输出
    codex/
      runner.go             启动 codex exec --json 子进程
      parser.go             JSONL 解析器，处理 codex 输出
...
lazycoding 通过 `Agent` 接口支持多个 AI 编程代理后端。后端通过 `agent.backend` 配置字段选择（`claude`、`opencode` 或 `codex`）。每个后端都有各自的 runner 和 parser，适配具体 CLI 工具的输出格式。
...
- 同一项目目录的多个 Telegram 对话（例如手机私聊和桌面群组）共享同一个 agent 会话，请求之间自动串行化。
- 群组中所有成员共享同一个 agent 会话，可以看到彼此的进展，适合团队协作。
...
lazycoding 支持三个 AI 编程代理后端，每个后端都有各自的 CLI 调用方式和输出格式：
...
通用特性：
- **结构化输出**：所有后端都输出 JSONL 格式（每行一个 JSON 对象），由对应的 parser 解析为 `agent.Event` 流。
- **会话恢复**：`--resume` 或 `--session` 参数支持续接现有会话。
- **进程管理**：`exec.CommandContext` 在 context 取消时（超时或 `/cancel`）保证 SIGKILL 子进程。
- **错误处理**：stderr 被捕获，若非空则附加到错误消息中展示给用户。
- **缓冲区大小**：Scanner 缓冲区 4 MB，避免大型工具输出在解析层被截断。

Claude 的 `parser.ParseLineMulti` 能处理单次 assistant 回复中包含多个 block（text + tool_use）的情况。OpenCode 和 Codex 的 parser 适配各自独特的 JSON 事件格式。
...
- 同一目录的多个 Telegram 对话共享一个 agent 会话。
...
以工作目录路径为 key，意味着同一项目目录的所有 Telegram 对话共享同一个 agent 会话——无论是私聊、群组，还是手机和桌面客户端各一个对话。这与团队共享代码库的工作模式一致。
